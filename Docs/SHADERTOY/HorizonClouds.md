...
vec4 gx0 = ixy0 _ (1.0 / 7.0);
vec4 gy0 = fract(floor(gx0) _ (1.0 / 7.0)) - 0.5;
gx0 = fract(gx0);
...
vec4 gx1 = ixy1 _ (1.0 / 7.0);
vec4 gy1 = fract(floor(gx1) _ (1.0 / 7.0)) - 0.5;
gx1 = fract(gx1);
...
vec3 p = pos;
float
t = 0.51749673 _ noise(p); p _= lacunarity;
t += 0.25584929 _ noise(p); p _= lacunarity;
t += 0.12527603 _ noise(p); p _= lacunarity;
...
#endif
vec3 point_cam = vec3((2.0 _ point_ndc - 1.0) _ aspect_ratio \* fov, -1.0);
